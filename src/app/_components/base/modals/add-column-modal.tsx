// src/app/_components/base/modals/add-column-modal.tsx
"use client";
...
export function AddColumnModal({
  ...
  return (
    <Popover
      ...
      className="w-116 overflow-hidden"
      dependency={isConfiguring} // Re-evaluate collision if view changes
      overlayStyle={{
        background: anchorEl ? "transparent" : "rgba(0,0,0,0.4)",
      }}
    >
      {!isConfiguring ? (
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2 border-b border-gray-100 p-2">
            <div className="flex w-full items-center gap-3 rounded border border-transparent bg-gray-100 px-3 py-2 focus-within:bg-white focus-within:ring-1 focus-within:ring-blue-600">
          ...
          <div className="max-h-120 overflow-y-auto p-2">
            {filteredFields.length > 0 ? (
              <>
                {agentFields.length > 0 && (
                  <div className="mb-4">
                    ...
                    <div
                      className={`grid gap-1 ${searchQuery ? "grid-cols-1" : "grid-cols-2"}`}
                    >
                      {agentFields.map((field) => (
                        <button
                          ...
                          className="flex items-center gap-3 rounded-lg border border-transparent px-3 py-2 text-left text-sm hover:bg-gray-50 focus:ring-2 focus:ring-blue-100 focus:outline-none"
                        >
                    ...
                    <div className="space-y-0.5">
                      {standardFields.map((field) => (
                        <button
                          ...
                          className="flex w-full items-center gap-3 rounded-lg border border-transparent px-3 py-2 text-sm hover:bg-gray-50 focus:ring-2 focus:ring-blue-100 focus:outline-none"
                        >
              ...
              <div className="flex flex-col items-center justify-center px-6 py-12 text-center text-gray-600">
                <p>No field types matching &quot;{searchQuery}&quot;</p>
                <button
                  onClick={() => {
                    setSearchQuery("");
        ...
        <div className="flex flex-col">
          <div className="space-y-3 px-4 py-2">
            <input
              ...
              className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-600 focus:ring-1 focus:ring-blue-600 focus:outline-none"
            />
            <button
              onClick={() => setIsConfiguring(false)}
              disabled={editColumn?.primary}
              className="flex w-full items-center justify-between rounded border border-gray-300 px-3 py-2 text-sm hover:bg-gray-100 focus:ring-1 focus:ring-blue-600 focus:outline-none disabled:opacity-75"
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-gray-900">{selectedFieldType?.name}</span>
              </div>
            ...
            <div className="mt-4 mb-2 flex items-center justify-between">
              <button className="flex items-center gap-2 rounded-md px-3 py-1.5 text-[13px] font-medium text-gray-600 hover:bg-gray-100">
                <PlusIcon className="inline h-4 w-4" /> Add description
              </button>
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="rounded-md bg-blue-600 px-3 py-1.5 text-[13px] text-white disabled:opacity-50"
                >
