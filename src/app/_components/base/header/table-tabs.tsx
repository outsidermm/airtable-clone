...
export function TableTabs({ base, tables, iconColor }: TableTabsProps) {
  ...
  return (
    <div className={`flex items-end ${getLightColorClass(iconColor)}`}>
      {/* role="tablist" with aria-label satisfies the WAI-ARIA tabs pattern.
          Each table is a tab panel switch — active table drives all grid queries. */}
      <div role="tablist" aria-label="Tables" className="flex flex-1 items-end gap-0">
        {tables.map((table, index) => {
          ...
          return (
            <div key={table.id} className="relative flex items-end">
              ...
              >
                {/* Single-click switches the active table; double-click initiates
                    inline rename. The 200ms timeout disambiguates the two gestures. */}
                <button
                  role="tab"
              ...
              {deleteConfirmTableId === table.id && (
                <>
                  ...
                  <div
                    role="alertdialog"
                    aria-modal="true"
                    aria-label="Delete table confirmation"
                    className="absolute top-full left-0 z-100 mt-0.5 w-72 rounded-lg border border-gray-200 bg-white p-4 shadow-lg"
                  >
        ...
        <div className="relative flex items-center">
          <button
            ...
            aria-expanded={isTableSearchOpen}
